...
live_design! {
    ...
    pub DrawSurface3D = {{DrawSurface3D}} {
        fn pixel(self) -> vec4 {
            let uv = self.pos;
        ...
        fn apply_colormap(self, t: float) -> vec3 {
            let t = clamp(t, 0.0, 1.0);

            // Colormap selection based on colormap_type uniform
        ...
        fn colormap_viridis(self, t: float) -> vec3 {
        ...
        fn colormap_plasma(self, t: float) -> vec3 {
        ...
        fn colormap_inferno(self, t: float) -> vec3 {
        ...
        fn colormap_cool_warm(self, t: float) -> vec3 {
        ...
        fn colormap_turbo(self, t: float) -> vec3 {
    ...
    pub DrawWireframe3D = {{DrawWireframe3D}} {
        fn pixel(self) -> vec4 {
            let uv = self.pos;
    ...
    // 3D bar/column with lighting
    pub DrawBar3D = {{DrawBar3D}} {
        fn pixel(self) -> vec4 {
            let uv = self.pos;

            // Determine which face we're on based on face_type uniform
            // 0 = top, 1 = front, 2 = right side

            // Base color from colormap
            let t = clamp(self.data_value, 0.0, 1.0);
            let base_color = self.apply_colormap(t);

            // Different shading per face
            let brightness = 0.5;
            if self.face_type < 0.5 {
                // Top face - brightest
                let brightness = 1.0;
                return vec4(base_color * brightness, 1.0);
            }
    ...
    pub DrawAxis3D = {{DrawAxis3D}} {
        ...
        fn point_in_triangle(self, p: vec2, v0: vec2, v1: vec2, v2: vec2) -> float {
...
pub struct DrawGrid3D {
    ...
    #[live(10.0)] pub grid_divisions: f32,
...
impl DrawAxis3D {
    ...
    pub fn draw_axis(&mut self, cx: &mut Cx2d, rect: Rect, color: Vec4) {
        self.color = color;
        self.draw_abs(cx, rect);
    }
}
